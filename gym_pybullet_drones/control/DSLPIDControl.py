...
class DSLPIDControl(BaseControl):
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL == DroneModel.RACE:
            ...
            self.P_COEFF_FOR = np.array([1.5, 1.5, 2.1])
            self.I_COEFF_FOR = np.array([.05, .05, .01])
            self.D_COEFF_FOR = np.array([1.0, 1.0, 1.5])
            self.P_COEFF_TOR = np.array([78000., 78000., -150000.])
            self.I_COEFF_TOR = np.array([0, 0, 800])
            self.D_COEFF_TOR = np.array([45000, 45000, -360000])
    ...
    def compute_car_pid_control(self, control_timestep, current_state, next_position):
        """
        ...
        # PID controller gains (tuned for more realistic response)
        Kp_v = 1  # Proportional gain for velocity 
        Ki_v = 0.09   # Integral gain for velocity
        Kd_v = 0.1   # Derivative gain for velocity

        Kp_psi = 0.01  # Proportional gain for steering 
        Ki_psi = 0.7   # Integral gain for steering
        Kd_psi = 0.9  # Derivative gain for steering

        # Time step
        delta_t = control_timestep #self.CTRL_TIMESTEP
        ...
        dx = next_position[0] - current_state[0]
        dy = next_position[1] - current_state[1]
        distance = np.sqrt(dx**2 + dy**2)
        
        # Compute desired heading (theta) to the goal
        theta_goal = np.arctan2(dy, dx)

        # PID control for velocity (v)
        error_v = distance
        error_v_integral += error_v * delta_t
        error_v_derivative = (error_v) / delta_t
        v = Kp_v * error_v + Ki_v * error_v_integral + Kd_v * error_v_derivative

        # Limit velocity to prevent runaway speed
        v = np.clip(v,0,2)

        # PID control for steering (psi)
        error_psi = theta_goal - current_state[2]
        error_psi_integral += error_psi * delta_t
        error_psi_derivative = (error_psi) / delta_t
        psi = Kp_psi * error_psi + Ki_psi * error_psi_integral + Kd_psi * error_psi_derivative
