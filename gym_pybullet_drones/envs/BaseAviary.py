...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
                 ...
                 NUM_CARS=0, INIT_XYZS_C=None, INIT_RPYS_C=None
                 ):
        ...
        self.CAR = True
        # self.NUM_CARS = 1   
        # self.INIT_XYZS_C = np.array([[0,0,0]])
        # self.INIT_RPYS_C = np.array([[0,0,0]])
        # self.target = None  # self.target can be defined here instead of None and can also be directly defined in test.py (as done now). Same goes for INIT_XYZS_C and RPYS. Also, Since self.target is not defined anywhere else in other classes, therefore it is initialized as None here

        self.NUM_CARS = NUM_CARS
        self.target = None #target if target is not None else np.zeros((NUM_CARS, 3))
        self.INIT_XYZS_C = INIT_XYZS_C if INIT_XYZS_C is not None else np.zeros((NUM_CARS, 3))
        self.INIT_RPYS_C = INIT_RPYS_C if INIT_RPYS_C is not None else np.zeros((NUM_CARS, 3))
        

        self.CAR_MODEL = DroneModel.CAR
        self.URDF_car = self.CAR_MODEL.value + ".urdf"
        #### Parameters ############################################
        ...
        if initial_xyzs is None:
            ...
            # self.INIT_XYZS_C = initial_xyzs_c
        else:
    ...
    def reset(self,
        ...
        ################ initial state ################
        self.current_state = self.update_car_states()  # Update the car's state
    ...
    def step(self,
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            if self.CAR is True:
            # targetVelocity, steeringAngle = self._processcarAction(self.target)
                targetVelocities = []
                steeringAngles = []
                for i in range(self.NUM_CARS):
                    target_pos = self.target[i]  # Get the target for car i
                    targetVelocity, steeringAngle = self._processcarAction(target_pos)
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):
            ...
            #### Step the simulation of CAR using the desired physics update ##
            if self.CAR is True:
                for i in range (self.NUM_CARS):
                    if self.PHYSICS == Physics.PYB:
                        self._physics_car(self.NUM_CARS,targetVelocity, steeringAngle)
    ...
    def _housekeeping(self):
        ...
        ######################################### Defining race car######################
         #### Initialize the raecar kinemaatic information ##########
        self.pos_c = np.zeros((self.NUM_CARS, 3))
        self.quat_c = np.zeros((self.NUM_CARS, 4))
        self.rpy_c = np.zeros((self.NUM_CARS, 3))
        self.vel_c = np.zeros((self.NUM_CARS, 3))
        self.ang_v_c = np.zeros((self.NUM_CARS, 3))

        self.CAR_IDS = np.array([p.loadURDF(pkg_resources.resource_filename('gym_pybullet_drones', 'assets/'+self.URDF_car),
                                              self.INIT_XYZS_C[i,:],
                                              p.getQuaternionFromEuler(self.INIT_RPYS_C[i,:]),
                                              flags = p.URDF_USE_INERTIA_FROM_FILE,
                                              physicsClientId=self.CLIENT
                                              ) for i in range(self.NUM_CARS)])
    ...
    def update_car_states(self,):
        """Update the positions, velocities, and orientations of all cars."""
        for i in range(self.NUM_CARS):
            self.pos_c[i], self.quat_c[i] = p.getBasePositionAndOrientation(self.CAR_IDS[i], physicsClientId=self.CLIENT)
            self.rpy_c[i] = p.getEulerFromQuaternion(self.quat_c[i])
            self.vel_c[i], self.ang_v_c[i] = p.getBaseVelocity(self.CAR_IDS[i], physicsClientId=self.CLIENT)
            # target_position = self.goal_position[i]
            current_state = [self.pos_c[i][0], self.pos_c[i][1], self.rpy_c[i][2]]
    ...
    def _physics_car(self,nth_car,targetVelocity, steeringAngle):
        
        # # exporting positions of car form gympybullet in a .txt file
        # folder_path = "/home/dzmitry/IndiaGrant/rl_environment/gym_pybullet_drones/StatusReviewResults"  # Update this path to your custom folder
        # file_name = os.path.join(folder_path, "robot_positions.txt")
        # with open(file_name, mode='a') as file:
        #     if file.tell() == 0:
        #         file.write("Car_ID x_position y_position\n")  # Add a header for clarity

        for nth_car in range(len(self.CAR_IDS)):

        #     self.update_car_states()  # Update the car's state
        #     target_position = self.goal_position[nth_car]
            # Your custom logic to compute control inputs for the car
            # current_state = [self.pos_c[nth_car][0], self.pos_c[nth_car][1], self.rpy_c[nth_car][2]]
            # x_pos = current_state[0]  # Get the x position
            # y_pos = current_state[1]  # Get the y position
            # # Write the positions to the .txt file
            # # file.write(f"{nth_car} {x_pos} {y_pos}\n")  # Write car_id and its x, y car positions


            # Apply the control inputs to the car's wheels
            self.inactive_wheels = [5, 7]
            self.wheels = [2, 3]
            self.maxForce = 5
            self.steering = [4, 6]
            for wheel in self.inactive_wheels:
                p.setJointMotorControl2(self.CAR_IDS[nth_car-1], wheel, p.VELOCITY_CONTROL, targetVelocity=0, force=0)

            for wheel in self.wheels:
                p.setJointMotorControl2(self.CAR_IDS[nth_car-1],
                                        wheel,
                                        p.VELOCITY_CONTROL,
            ...
            for steer in self.steering:
                p.setJointMotorControl2(self.CAR_IDS[nth_car-1], steer, p.POSITION_CONTROL, targetPosition=steeringAngle)
    ...
    def _processcarAction(self, target
                          ):
        """Pre-processes the action passed to `.step()` into motors' RPMs.
