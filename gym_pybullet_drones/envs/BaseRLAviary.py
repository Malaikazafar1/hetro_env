...
class BaseRLAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        if act in [ActionType.PID, ActionType.VEL, ActionType.ONE_D_PID]:
            ...
            if drone_model in [DroneModel.CF2X, DroneModel.CF2P, DroneModel.RACE, DroneModel.CAR]:
                if drone_model in [DroneModel.CF2X, DroneModel.CF2P]:
                    self.ctrl = [DSLPIDControl(drone_model=DroneModel.CF2X) for i in range(num_drones)]
                elif drone_model == DroneModel.RACE:
                    self.ctrl = [DSLPIDControl(drone_model=DroneModel.RACE) for i in range(num_drones)]
            else:
        ...
        if act == ActionType.VEL:
            self.SPEED_LIMIT = 0.035 * self.MAX_SPEED_KMH * (1000/3600)
    ...
    def _actionSpace(self):
        ...
        if self.ACT_TYPE in [ActionType.RPM, ActionType.VEL]:
            ...
            exit()
        act_lower_bound = np.array([-1.0*np.ones(size) for i in range(self.NUM_DRONES)])
        act_upper_bound = np.array([+1.0*np.ones(size) for i in range(self.NUM_DRONES)])
        #
        ...
        for k in range(action.shape[0]):
            ...
            if self.ACT_TYPE == ActionType.RPM:
                ...
                rpm_k, _, _ = self.ctrl[k].computeControl(control_timestep=self.CTRL_TIMESTEP,
                                                        ...
                                                        target_pos=state[0:3]+0.1*target#next_pos
                                                        )
                ...
                temp, _, _ = self.ctrl[k].computeControl(control_timestep=self.CTRL_TIMESTEP,
                                                        ...
                                                        target_rpy=np.array([0,0,state[9]]), # keep current yaw state[9] ==>target[4]
    ...
    def _processcarAction(self, target):
        ...
        self.ctrl_car = DSLPIDControl(drone_model=DroneModel.RACE)
        target_position = target
        current_state = self.update_car_states()
        targetVelocity, steeringAngle = self.ctrl_car.compute_car_pid_control(self.CTRL_TIMESTEP, 
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            # ret = np.hstack([ret,
            #     np.array([[self.action_buffer[i][j, :] for i in range(self.ACTION_BUFFER_SIZE)] for j in range(self.NUM_DRONES)]).reshape(self.NUM_DRONES, -1)
            # ])
            return ret
            ############################################################
        else:
