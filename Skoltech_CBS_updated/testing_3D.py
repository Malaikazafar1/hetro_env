...
def min_distance_subarrays(arrays, point, d):
    ...
    for subarray in arrays:
        distances = [np.sqrt((x - px) ** 2 + (y - py) ** 2 + (z - pz) ** 2) for x, y, z in subarray]
...
for i in range(1, len(high_level.agents)):
    a,b = solution = high_level.find_path_3_D(
        agent_num=i,
...
while open_list != []:
    ...
    for i in range(2):
        ...
        the_solution = next_node.find_path_3_D(
            agent_num=agent,
...
if obstacle_list != []:
        ...
        for i in obstacle_distance_dictionary:
            ...
            obstacle_distance_dictionary[i] = min_distance_subarrays(truth_values, location_list[i], 1/2)
